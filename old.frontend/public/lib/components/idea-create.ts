...
export class IdeaCreate extends LitElement {
  ...
  private async _createClick() {
    this.content = '';
    if (this.shadowRoot) {
      const ttt = this.shadowRoot.getElementById('idea') as HTMLInputElement;
      this.content = ttt.value.trim();
      ttt.value = ''; // reset input
    }
    if (this.content.length > 0) {
      const event = new CustomEvent<string>('pressed-create', {
        detail: this.content,
      });
  ...
  override render() {
    return html`
      ...
      <button id="add-idea" @click="${() => this._createClick()}">
        hinzufügen
      </button>
    `;
  }
